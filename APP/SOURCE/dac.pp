...
#line 32 "G:\workspace\EEA\V615\APP\SOURCE\INCLUDE\DAC.H"
...
void Write_DAC(int n)
{
	...
	{ 
		...
		{ 
			...
			{
				{asm("	ldi		62h,IOF		");		 }; 
				*((volatile unsigned int*) 0x808048) = i|0x4000; 
		...
		if ((REG_DAC[n].STAT & 0x00000008) && (n<9))
		{ 
	...
	if (ext_mb==0)
	{
		...
		r = Write_ADC_DAC(addr, 3, (unsigned char*)pt);	 
		
		if (!r)
			VAR_NaN(&REG_DAC[n]);
		else  
			REG_DAC[n].STAT &= 0x00000008 ^ 0xFFFFFFFF;
		
		if ((REG_DAC[n].STAT & 0x00000008) && (n<=8))
		{
			VAR_NaN(&AO[n-0].CURRENT);
			VAR_NaN(&AO[n-0].CURRENT_PERCENT_OF_RANGE);
			AO[n-0].present = 0;
		}
	}
...
void MB_RX_DAC_Aout(int id, int i)
{
	if (!PORT[id].failed)
	{
		REG_DAC[i].STAT 		&= 0x00000008 ^ 0xFFFFFFFF;
		AO[i-0].present  = 1;
	}
	else
	{
		VAR_NaN(&REG_DAC[i]);
		VAR_NaN(&AO[i-0].CURRENT);
		VAR_NaN(&AO[i-0].CURRENT_PERCENT_OF_RANGE);
		AO[i-0].present = 0;
	}
}
...
void Setup_DAC(int n)
{
	...
	{ 
		switch(n)
		{
			...
			{
				{asm("	ldi		62h,IOF		");		 }; 
				*((volatile unsigned int*) 0x808048) = 0x9001; 
				{	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
...
void Set_Current(void)
{
	...
	{ 
		...
		if (((i+0)>=5) && ((i+0)<=8) && (AO[i].v.val==-2)) 
			AO[i].v.val = -1;  

		minc = VAR_Get_Unit_Param(&AO[i].CURRENT, 0x00060000, 0, 0);
		maxc = VAR_Get_Unit_Param(&AO[i].CURRENT, 0x00050000, 0, 0);
		minv = AO[i].TRIM_MIN.calc_val*(float)DAC_TRIM_F_1[i+0]/DAC_TRIM_I_1[i+0];
		maxv = AO[i].TRIM_MAX.calc_val*(float)DAC_TRIM_F_1[i+0]/DAC_TRIM_I_1[i+0];
...
#line 444
			...
			{ 
				AO[0].MANUAL = 1;
				VAR_Update(&AO[0].CURRENT, VAR_Get_Unit_Param(&AO[0].CURRENT, 0x00060000, 0, 0), 0, 0);
			}
			...
			else if ((v->STAT & (0x00000008|0x00000010|0x00000020|0x00000040|0x00000080)) || (v==&REG_DUMMY))
			{ 
				
				if ((v->STAT & 0x00000008) || (v==&REG_DUMMY))
	 			{
				...
				else if (v->STAT & (0x00000020|0x00000080))	
	 				VAR_Update(&AO[i].CURRENT, minc, 0, 0);
				else if (v->STAT & (0x00000010|0x00000040)) 
	 				VAR_Update(&AO[i].CURRENT, maxc, 0, 0);
...
void PID_Init(int loop)
{
    ...
    AO[loop].pid_lastcycle  = *((volatile unsigned int*) 0x808034);
}
...
#line 594
	...
	AO[loop].pid_lastcycle 	= c;	 
	 
	
		
		
		
	AO[loop].pid_propband	= AO[loop].P.val;	

	if (AO[loop].pid_propband > 1000.0) 			 
		AO[loop].pid_propband = 1000.0;
		
    P = AO[loop].pid_propband;		 

	 
#line 635
    if (AO[loop].I.val <= 0.0) 		 
		AO[loop].I.val  = 10000.0;	 
    
	I = AO[loop].I.val;				 

    if (AO[loop].D.val < 0.0) 		 
		AO[loop].D.val = 0.0;   	
    
	D  = AO[loop].D.val;			 
	
	last_e  = AO[loop].pid_error;	
	
	
	sp = AO[loop].PID_setpoint;		
	
	e = AO[loop].PID_input - sp;	
