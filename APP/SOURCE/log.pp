...
{
	{ 
		SYS.log_end			= FLASH_SIZE() + (unsigned int)((unsigned int*)0x400000);
		SYS.log_start 		= ((unsigned int*)0x470000);
		SYS.log_len 		= SYS.log_end - (unsigned int)SYS.log_start;
		SYS.log_sector_size = 0x00008000;
		SYS.erased_sector 	= 0;
		SYS_LOG_EVENT 		= 0;
		USER_LOG_EVENT 		= 0;
	...
	{ 
		...
		CCM_Records.log_end 		= (unsigned int)((unsigned int*)0x402000) + 2*0x00001000;
		CCM_Records.log_start 		= (unsigned int*)((unsigned int*)0x402000);
		CCM_Records.log_len 		= CCM_Records.log_end - (unsigned int)CCM_Records.log_start;
		CCM_Records.log_sector_size = 0x00001000;
		CCM_Records.erased_sector 	= 0;
		...
		else
		{
			...
			{
				CCM_Records.log_record_size 		= 18;
		...
		CCM_Records.LOG_ERASE = 0;
	}

	{ 
		FC_Periodic.log_end 		= (unsigned int)((unsigned int*)0x404000) + 4*0x00001000;
		FC_Periodic.log_start 		= (unsigned int*)((unsigned int*)0x404000);
		FC_Periodic.log_len 		= FC_Periodic.log_end - (unsigned int)FC_Periodic.log_start;
		FC_Periodic.log_sector_size = 0x00001000;
...
void EraseSector(LOG* log, unsigned int s)
{
	...
	while(s >= log->log_n_sectors)	 
		s -= log->log_n_sectors;
	
	FLASH_ERASE_BLK(log->log_start + (s*log->log_sector_size));
	FLASH_LOCK_BLK(log->log_start + (s*log->log_sector_size), 1);
...
void MarkLogStart(LOG* log, unsigned int s)
{
	...
	*((unsigned int*)0x400000) 	= 0x00400040;
	p[0] 		= i;
	*((unsigned int*)0x400000) 	= 0x00700070;
	status 		= *((unsigned int*)0x400000);
	
	while (!( (status & 0x00800080) == 0x00800080) )
		status = *((unsigned int*)0x400000);
	
	*((unsigned int*)0x400000) = 0x00FF00FF;
...
void RecordLogEntry(LOG* log, int mode, unsigned int* p)
{
	...
	{ 
		...
		while(log->log_current_sector >= log->log_n_sectors)	 	
	...
	{ 
		...
		FLASH_LOCK_BLK(log->log_start + (log->log_sector_size*log->log_current_sector), 0);
		...
		*((unsigned int*)0x400000) = 0x00FF00FF;
		...
		FLASH_LOCK_BLK(log->log_start + (log->log_sector_size*log->log_current_sector), 1);

		for (i=0;i<log->log_record_size;i++)
		{ 
			if (addr[i] != p[i])
				DIAGNOSTICS[1] |= 0x0100;
		}
...
BOOL DataLogCounter(int i, BOOL FORCE)
{
	...
	if ( (i==0) && ((ANALYZER_MODE.val&0xFF)==6) )	 
		rec[6] = TOIEEE(CCM_GAS_TOTAL.val);
	
	RecordLogEntry(&FC_Periodic, 0x0B+i, rec);
...
void DataLog(void)
{
	...
	if (SYS_LOG_ENABLED & 0x01 && (SYS.log_len>0))
	{ 
		SYS_LOG_EVENT = 1;

		RecordLogEntry(&SYS, 0x01, rec);
		Timer_Create(&TMR_DATALOG, SYS_log_sample_period.val, 1, (int*)DataLog, 0);
		Timer_Insert(&TMR_DATALOG,0,1);
		Timer_Insert(&TMR_CLEAR_SYS_LOG_EVENT_FLAG,0,1);
	}
	else
	{ 
		SYS_LOG_EVENT = 0;
...
void EraseEntireLog(LOG* log)
{
	...
	for (i=0;i<log->log_n_sectors;i++)
	{
		...
		FLASH_LOCK_BLK(log->log_start+(i*log->log_sector_size), 1);
	}
...
unsigned int* FindLastLogRecord(LOG* log, int mode)
{
	unsigned int* p;									 
	unsigned int* start;								 
	unsigned int ck,cksum, log_n, log_current_sector;	 
	int i;												 
