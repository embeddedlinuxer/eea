...
void Initialize_Variables(void)
{
	...
	{ 
		...
		{
			IDEC_SA[i].val  = 0;
            IDEC_SA[i].vect = (int*)0;
		}

		IDEC_SA[1].val 		= 0x0081;				 
        IDEC_SA[0].val 		= 1;			 
	...
	if ((*((volatile char*) 0x0900001) & 0x10)) 
		DIPSWITCH[0] = 0;
	else 
		DIPSWITCH[0] = 1;

	if ((*((volatile char*) 0x0900001) & 0x20)) 
		DIPSWITCH[1] = 0;
	else 
		DIPSWITCH[1] = 1;
	...
	{ 
		...
		VAR_Initialize(&COR[i].damp_flow_rate, 70, 51, 100.0, 100.0, 0x00000004|0x00000002);
		...
		VAR_Initialize(&COR[i].damp_density, 70, 51, 100.0, 100.0, 0x00000004|0x00000002);
		...
		VAR_Initialize(&COR[i].mfr_cutoff, 72, 251, 100.0, 1000.0, 0x00000004|0x00000002);
		...
		VAR_Initialize(&COR[i].vfr_cutoff, 66, 251, 100.0, 1000.0, 0x00000004|0x00000002);
		...
		VAR_Initialize(&COR[i].mcf, 72, 251, 100.0, 100000.0, 0x00000004|0x00000002);
        ...
        VAR_Initialize(&COR[i].address, 0, 251, 1.0, 1.0, 0x00000004|0x00000100);
		sprintf(COR[i].address.name, "COR%i Address", i+1);
		COR[i].address.aux = (i<<16)|(313);
		VAR_Update(&COR[i].address, 1.0, 0, 0);
		COR[i].address.vect2 = (int*)CORIOLIS_INT_CFG;        
		
		 
		VAR_Initialize(&COR[i].mass_unit, 0, 251, 1.0, 1.0, 0x00000004|0x00000002);
		...
		VAR_Initialize(&COR[i].density_cutoff, 0, 251, 100.0, 1000.0, 0x00000004|0x00000002);
		...
		COR[i].u_density 				= 92;
		COR[i].density_corrected 		= -99.0;
		COR[i].density_PDI_corrected 	= 0;
		COR[i].u_density_corrected 		= 105;
		COR[i].u_P 						= 6;
		COR[i].P 						= 0;
		COR[i].T 						= 0;
		COR[i].u_T 						= 33;
...
{	
	...
	{
		if (HW_VER>=0xB7)
			SRAM_SCRATCHPAD = ((unsigned int*)0xA38000);
		else
			SRAM_SCRATCHPAD = ((unsigned int*)0xB00000);
	}
	else
		SRAM_SCRATCHPAD = ((unsigned int*)0xA38000);
	...
	*((volatile unsigned int*) 0x808064)  = (0x0018 | 0x00A0);	 
	...
	{{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900006) &= (0xFF ^ 0x02);{asm("	pop		ST			");};};
	{{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900006) &= (0xFF ^ 0x04);{asm("	pop		ST			");};};
	...
	s = (unsigned int*) ((unsigned int*)0x400000);

	if ( (d >= s) && (d < (s + 0x00400000)) )
		MEM = 0;
	else
		MEM = 1;
	...
	sprintf(LCD_DISPLAY[0],"Phase Dynamics, Inc.");	
	sprintf(LCD_DISPLAY[1],"Initializing Data...");	
	sprintf(LCD_DISPLAY[2],"HW Version:    %2d.%02d", hwu, hwl);	
	sprintf(LCD_DISPLAY[3],"FW Version:  %4.2f.%02i", fw, i);	
#line 685
	LCD_PAGE();
	MENU_INIT();
	...
	{
		case 0:		sprintf(LCD_DISPLAY[1]," Low Range Analyzer "); break;
		case 3:		sprintf(LCD_DISPLAY[1]," Mid Range Analyzer "); break;
		case 4:		sprintf(LCD_DISPLAY[1],"High Range Analyzer "); break;
		case 1:		sprintf(LCD_DISPLAY[1],"Full Range Analyzer "); break;
		case 6:		sprintf(LCD_DISPLAY[1],"  CCM Application   "); break;
		case 2:	sprintf(LCD_DISPLAY[1],"      Analyzer      "); break;
		default: break;
	}
#line 739
	LCD_PAGE();
	...
	if ((REG_TEMPERATURE_EXTERNAL.unit & 0xFF) == 32)
		DIO_TEMP_C.val = 1;
	else
		DIO_TEMP_C.val = 0;
	...
	{{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900006) |= 0x02;{asm("	pop		ST			");};};
	{asm("	or		0002h, IE");};
	{asm("	andn	0002h, IF");};
	 

	{{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900004) |= 0x10;{asm("	pop		ST			");};}; 

	if (LOG_STATUS == (82))
	{
		Timer_Insert(&TMR_ALFAT_LOG,0,1);
	}
