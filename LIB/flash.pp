#line 124 "Y:\workspace_eea\V615\APP\SOURCE\INCLUDE\GLBLS.H"
...
#line 89 "Y:\workspace_eea\V615\INCLUDE\I2C.H"
...
#line 171 "Y:\workspace_eea\V615\APP\SOURCE\INCLUDE\GLBLS.H"
...
unsigned int FLASH_SIZE(void)
{
	...
	r[0] 		= 0;
	r[1] 		= 0;
	*((unsigned int*)0x400000) 	= 0x00900090;
	dev_id 		= ((unsigned int*)0x400000)[1];
...
int FLASH_CHECK_ADDR(unsigned int* addr, unsigned int n)
{
 	...
 	if (addr >= (((unsigned int*)0x400000)+f)) 
		return 0x20;
 	
	if (addr < ((unsigned int*)0x400000)) 
		return 0x20;

 	if (addr < (((unsigned int*)0x400000)+0x00008000))
 	{ 
	 	if (n==0)
	 	{
	 		if ((((unsigned int)addr-(unsigned int)((unsigned int*)0x400000))%0x00001000) == 0)
	 			return 0;
	 	}
		if (n>0x00001000) 
			return 0x20;
		
		if (((unsigned int)addr+n-1) >= ((unsigned int)((unsigned int*)0x400000)+0x00008000)) 
			return 0x20;
		
		mask = 0xFFFFFFFF ^ (0x00001000-1);
		f 	 = ((unsigned int)addr&mask) + 0x00001000;
 	...
 	else
 	{
	 	if (n==0)
	 	{
			if ((((unsigned int)addr-(unsigned int)((unsigned int*)0x400000))%0x00008000) == 0)
				return 0;
		}
		...
		if (((unsigned int)addr+n-1) >= ((unsigned int)((unsigned int*)0x400000)+f)) 
			return 0x20;
		
		mask = 0xFFFFFFFF ^ (0x00008000-1);
		f 	 = ((unsigned int)addr&mask) + 0x00008000;
...
int FLASH_ERASE_BLK(unsigned int* addr)
{
	...
	{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");}; {{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900004) &= (0xFF ^ 0x08);{asm("	pop		ST			");};};

	FLASH_LOCK_BLK(addr, 0);

	*((unsigned int*)0x400000) 	= 0x00200020;
	addr[0] 	= 0x00D000D0;
...
int FLASH_STATUS(void)
{
	...
	*((unsigned int*)0x400000) 	= 0x00700070;
	...
	while(1)
	{
		...
		{ 
			r 			= 0x01;
			*((unsigned int*)0x400000) 	= 0x00500050;
			break;
		}
...
void FLASH_READ_ESN_MFGR(unsigned int* dat)
{
	...
	*((unsigned int*)0x400000)	= 0x00900090;
	...
	for(i=0;i<4;i++)
		dat[i] = ((unsigned int*)0x400000)[i+0x81];
		
	 
	dat[4] 		= ((unsigned int*)0x400000)[0x80];	
	*((unsigned int*)0x400000) 	= 0x00FF00FF;
	{{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900004) |= 0x08; *((volatile char*) 0x0900004) |= 0x80;{asm("	pop		ST			");};}; 
	{asm("	pop		ST			");};
}
...
void FLASH_READ_ESN_USER(unsigned int* dat)
{
	...
	*((unsigned int*)0x400000) = 0x00900090;
	...
	for(i=0;i<4;i++)
		dat[i] = ((unsigned int*)0x400000)[i+0x85];
...
int FLASH_WRITE_ESN_USER(unsigned int* dat)
{
	...
	{{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900004) &= (0xFF ^ 0x08);{asm("	pop		ST			");};};
	*((unsigned int*)0x400000)	= 0x00900090;
	status 		= ((unsigned int*)0x400000)[0x80];
	*((unsigned int*)0x400000) 	= 0x00FF00FF;
	...
	for(i=0;i<4;i++)
	{
		*((unsigned int*)0x400000) 	= 0x00C000C0;
		dat[i] 		= ((unsigned int*)0x400000)[i+0x85];
		*((unsigned int*)0x400000) 	= 0x00700070;
		status 		= *((unsigned int*)0x400000);
...
int FLASH_LOCK_ESN_USER(void)
{
	...
	{{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900004) &= (0xFF ^ 0x08);{asm("	pop		ST			");};};
	*((unsigned int*)0x400000) 		= 0x00C000C0;
	((unsigned int*)0x400000)[0x80] = 0xFFFDFFFD;	 
	*((unsigned int*)0x400000) 		= 0x00700070;
	status 			= *((unsigned int*)0x400000);
