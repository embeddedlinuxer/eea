...
void Setup_ADC(int n)
{
	...
	if (SA)
	{
		...
		switch(n)
		{
			...
			{
				{asm("	ldi		26h,IOF		");		 };        		
				*((volatile unsigned int*) 0x808048) = 0xA000;	  
				{	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
				
				{asm("	ldi		26h,IOF		");		 }; 					 
				*((volatile unsigned int*) 0x808048) = ((0xA000 | 0x000 | 0x100 | 0x060 | 0x008 | 0x000 | 0x002) | 0x000); 
				{	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
                
                {{asm("	push	ST			"); 	 						 asm("	andn	2000h, ST	");};*((volatile char*) 0x0900006) |= 0x04;{asm("	pop		ST			");};};
			   	{asm("	andn	0004h, IE");};
			   	{asm("	andn	0004h, IF");};
   			...
   			case 10:
			{
				...
				{
					p[0] = 0x70;
					r 	 = Write_TMP75(1, 1, (unsigned char*)p);	 
		
					p[0] = 0x40;
					p[1] = 0x00;
					r 	 = Write_TMP75(2, 2, (unsigned char*)p);	 
		
					p[0] = 0xE7;
					p[1] = 0x00;
					r 	 = Write_TMP75(3, 2, (unsigned char*)p);	 
				}
...
void Read_ADC_test(void)
{
	...
	{ 
		{asm("	ldi		26h,IOF		");		 }; *((volatile unsigned int*) 0x808048) = (0xB000); {	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
	    TLV_REF[0] = SP0_RX_DATA & 0xFFF0;
	}

	{ 
		{asm("	ldi		26h,IOF		");		 }; *((volatile unsigned int*) 0x808048) = (0xD000); {	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
	    TLV_REF[1] = SP0_RX_DATA & 0xFFF0;
	}

	{ 
		{asm("	ldi		26h,IOF		");		 }; *((volatile unsigned int*) 0x808048) = (0xC000); {	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
...
void Read_ADC_FIFO(void)
{
	...
	{ 
		{asm("	ldi		26h,IOF		");		 }; 
		*((volatile unsigned int*) 0x808048) = (0xE000); 
		{	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
	...
	RTD_R 				= f;
	f 					= 0.1102750E-02 * 1.0 * 1.0 * f * f + 0.2333109E+01 * 1.0 * f + -0.2443125E+03;
	f 					= (f * TEMPERATURE_TRIM_T1) + TEMPERATURE_TRIM_T0;
	
	VAR_Update(&REG_ADC[2], f, 0, 0);

	if(HW_VER<0xB7)
	{ 
		ADC_INT[10] 	= a[2];
		f 					= ((float)ADC_INT[10] * ADC_TRIM_F_1[10] / ADC_TRIM_I_1[10]);
		



		f = 182.1493625*f - 273.15;
		
		VAR_Update(&REG_ADC[10], f, 0, 0);
	}
	else
	{ 
		ADC_INT[11] 	= a[2];
		f 					= ((float)ADC_INT[11]*ADC_TRIM_F_1[11]/ADC_TRIM_I_1[11]);
		
		VAR_Update(&REG_ADC[11], f, 0, 0);
	}
	
	 
	ADC_INT[1] 	= a[1];
	f 					= ((float)ADC_INT[1]*ADC_TRIM_F_1[1]/ADC_TRIM_I_1[1]);
	
	VAR_Update(&REG_ADC[1], f, 0, 0);

	 
	ADC_INT[0] 	= a[0];
	f 					= ((float)ADC_INT[0]*ADC_TRIM_F_1[0]/ADC_TRIM_I_1[0]);
	
	VAR_Update(&REG_ADC[0], f, 0, 0);
...
void Convert_ADC(void)
{
	...
	{
		{asm("	ldi		26h,IOF		");		 }; 
		*((volatile unsigned int*) 0x808048) = 0x0000; 
		{	SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;						asm("	tstb	0020h, IF	");						asm("	.word	6A05FFFEh	");						{asm("	andn	0020h, IF");};						{asm("	ldi		22h,IOF		");		 };						SP0_RX_DATA = *((volatile unsigned int*) 0x80804C) & 0xFFFF;};
	...
	switch(n)
	{
		...
		{ 
			...
			{ 
				p[0]	= 0x9C;	
				p[0]   |= ch;
				r 		= Write_ADC_DAC(addr, 1, (unsigned char*)p);	 
...
void ADC_UPDATE(void)
{
	...
	for (i=0;i<5;i++)
	{
		...
		minc 	= VAR_Get_Unit_Param(&AI[i].CURRENT, 0x00060000, 0, 0);
		maxc 	= VAR_Get_Unit_Param(&AI[i].CURRENT, 0x00050000, 0, 0);
		t 		= REG_ADC[i+5].val * 10.0;
		...
		{
			if (AI[i].CURRENT.STAT & 0x00000008)
				VAR_NaN(v);
			else
			{
				...
				if ( v == &REG_STREAM_SELECT )
				{
					...
					if ( (int)round(t,0) == (int)REG_STREAM_SELECT.val ) 
						continue;
				}                
				VAR_Update(v, t, 0,1);
	    	}
