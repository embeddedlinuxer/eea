...
;* Date/Time created: Mon Jan 31 15:51:16 2022                                *
;******************************************************************************
...
;******************************************************************************
;	C:\tic3x4x\c3x4x\cgtools\bin\ac30.exe -@_9TC.AAA 
	.file	"i2c.c"
...
;----------------------------------------------------------------------
	...
	.line	25
;----------------------------------------------------------------------
; 408 | Write_DS1340(8, 2, &RTC[0]); // Trickle Charger 0x08                   
;----------------------------------------------------------------------
        ldiu      @CL5,r3               ; |408| 
        ldp       @CL4,DP
        ldiu      8,ar2                 ; |408| 
	...
	.line	26
;----------------------------------------------------------------------
; 409 | Write_DS1340(7, 1, &RTC[9]); // FT Register     0x07                   
;----------------------------------------------------------------------
        ldp       @CL10,DP
        ldiu      @CL10,r3              ; |409| 
        ldp       @CL4,DP
...
;----------------------------------------------------------------------
        ...
        ldiu      @CL4,r0               ; |414| 
        ldiu      1,r2                  ; |414| 
        callu     r0                    ; far call to _Write_DS1340	; |414| 
                                        ; |414| Far Call Occurs
L79:        
	.line	34
;----------------------------------------------------------------------
...
;* r2    assigned to _p
	...
	.line	1
;----------------------------------------------------------------------
; 434 | BOOL Write_MC24C02_Record(int addr, unsigned char* p)                  
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      7,sp
	.line	2
;----------------------------------------------------------------------
; 436 | int i,j,k,r;    /* counter; counter; address error accumulator; return
;     |  */                                                                    
; 437 | BOOL ack;               /* transfer check
;     |                                                  */                    
;----------------------------------------------------------------------
        sti       r2,*+fp(2)            ; |435| 
        sti       ar2,*+fp(1)           ; |435| 
...
;----------------------------------------------------------------------
	...
	.line	19
;----------------------------------------------------------------------
; 452 | ack = I2C_TX_BYTE(addr);                /* address without read bit set
;     |  */                                                                    
;----------------------------------------------------------------------
        ldp       @CL2,DP
        ldiu      @CL2,r0               ; |452| 
        ldiu      *+fp(1),ar2           ; |452| 
        callu     r0                    ; far call to _I2C_TX_BYTE	; |452| 
                                        ; |452| Far Call Occurs
        sti       r0,*+fp(7)            ; |452| 
	.line	21
;----------------------------------------------------------------------
; 454 | if (!ack)                                               /* address erro
;     | r */                                                                   
;----------------------------------------------------------------------
        cmpi      0,r0                  ; |454| 
        bne       L92                   ; |454| 
;*      Branch Occurs to L92            ; |454| 
	.line	22
;----------------------------------------------------------------------
; 455 | k++;                                                                   
; 456 | else                                                                   
;----------------------------------------------------------------------
        ...
        sti       r0,*+fp(5)            ; |455| 
        bu        L89                   ; |433| 
;*      Branch Occurs to L89            ; |433| 
	.line	24
;----------------------------------------------------------------------
; 457 | break;                                          /* address OK, continue
;     |  */                                                                    
;----------------------------------------------------------------------
L89:        
	.line	26
...
;----------------------------------------------------------------------
	...
	.line	33
        bu        L86                   ; |466| 
;*      Branch Occurs to L86            ; |466| 
L92:        
	.line	35
;----------------------------------------------------------------------
; 468 | if (r==FALSE)                                           /* address fail
;     | ure, return */                                                         
;----------------------------------------------------------------------
        ldiu      *+fp(6),r0            ; |468| 
        cmpi      0,r0                  ; |468| 
        beq       L101                  ; |468| 
;*      Branch Occurs to L101           ; |468| 
	.line	36
;----------------------------------------------------------------------
; 469 | break;                                                                 
;----------------------------------------------------------------------
	.line	38
;----------------------------------------------------------------------
; 471 | ack = I2C_TX_BYTE(j*16);                        /* set the pointer */  
;----------------------------------------------------------------------
        ldiu      *+fp(4),ar2           ; |471| 
        ldp       @CL2,DP
        ash       4,ar2                 ; |471| 
        ldiu      @CL2,r0               ; |471| 
        callu     r0                    ; far call to _I2C_TX_BYTE	; |471| 
                                        ; |471| Far Call Occurs
        sti       r0,*+fp(7)            ; |471| 
	.line	40
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
        ...
        cmpi      16,r0                 ; |479| 
        bge       L99                   ; |479| 
;*      Branch Occurs to L99            ; |479| 
L96:        
	.line	48
;----------------------------------------------------------------------
; 481 | ack = I2C_TX_BYTE(p[j*16+i]);                                          
;----------------------------------------------------------------------
        ldiu      *+fp(4),ir0           ; |481| 
        ldp       @CL2,DP
        ash       4,ir0                 ; |481| 
        ldiu      *+fp(3),ar0           ; |481| 
        addi      *+fp(2),ir0           ; |481| Unsigned
        ldiu      @CL2,r0               ; |481| 
        ldiu      *+ar0(ir0),ar2        ; |481| 
        callu     r0                    ; far call to _I2C_TX_BYTE	; |481| 
                                        ; |481| Far Call Occurs
        sti       r0,*+fp(7)            ; |481| 
	.line	50
;----------------------------------------------------------------------
...
L98:        
	.line	46
        ...
        cmpi      16,r0                 ; |479| 
        blt       L96                   ; |479| 
;*      Branch Occurs to L96            ; |479| 
L99:        
	.line	57
;----------------------------------------------------------------------
; 490 | if (r==FALSE)                                           /* transfer fai
;     | lure, return */                                                        
;----------------------------------------------------------------------
        ldiu      *+fp(6),r0            ; |490| 
        cmpi      0,r0                  ; |490| 
        beq       L101                  ; |490| 
;*      Branch Occurs to L101           ; |490| 
	.line	58
;----------------------------------------------------------------------
; 491 | break;                                                                 
;----------------------------------------------------------------------
...
;* r2    assigned to _p
	...
	.line	1
;----------------------------------------------------------------------
; 515 | BOOL Read_MC24C02_Record(int addr, unsigned char* p)                   
;----------------------------------------------------------------------
        push      fp
        ldiu      sp,fp
        addi      7,sp
	.line	2
;----------------------------------------------------------------------
; 517 | int i,j,k,r;    /* byte counter; counter; address error accumulator; re
;     | turn */                                                                
; 518 | BOOL ack;               /* check I2C operations
;     |                                                  */                    
;----------------------------------------------------------------------
        sti       r2,*+fp(2)            ; |516| 
        sti       ar2,*+fp(1)           ; |516| 
...
;----------------------------------------------------------------------
	...
	.line	16
;----------------------------------------------------------------------
; 530 | ack = I2C_TX_BYTE(addr);                /* address without read bit set
;     |  */                                                                    
;----------------------------------------------------------------------
        ldp       @CL2,DP
        ldiu      @CL2,r0               ; |530| 
        ldiu      *+fp(1),ar2           ; |530| 
        callu     r0                    ; far call to _I2C_TX_BYTE	; |530| 
                                        ; |530| Far Call Occurs
        sti       r0,*+fp(7)            ; |530| 
	.line	18
;----------------------------------------------------------------------
; 532 | if (!ack)                                               /* accumulate a
;     | ddress errrors   */                                                    
;----------------------------------------------------------------------
        cmpi      0,r0                  ; |532| 
        bne       L112                  ; |532| 
;*      Branch Occurs to L112           ; |532| 
	.line	19
;----------------------------------------------------------------------
; 533 | k++;                                                                   
; 534 | else                                                                   
...
;----------------------------------------------------------------------
	...
	.line	30
        bu        L106                  ; |544| 
;*      Branch Occurs to L106           ; |544| 
L112:        
	.line	32
;----------------------------------------------------------------------
; 546 | if (r==FALSE)                                           /* address fail
;     | ure, return */                                                         
;----------------------------------------------------------------------
        ldiu      *+fp(6),r0            ; |546| 
        cmpi      0,r0                  ; |546| 
...
;----------------------------------------------------------------------
	.line	35
;----------------------------------------------------------------------
; 549 | ack = I2C_TX_BYTE(j*16);                        /* set the pointer */  
;----------------------------------------------------------------------
        ldiu      *+fp(4),ar2           ; |549| 
        ldp       @CL2,DP
        ash       4,ar2                 ; |549| 
        ldiu      @CL2,r0               ; |549| 
        callu     r0                    ; far call to _I2C_TX_BYTE	; |549| 
                                        ; |549| Far Call Occurs
        sti       r0,*+fp(7)            ; |549| 
	.line	37
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
	...
	.line	44
;----------------------------------------------------------------------
; 558 | ack = I2C_TX_BYTE(addr|0x01);           /* address with read bit set */
;----------------------------------------------------------------------
        ldp       @CL2,DP
        ldiu      1,ar2                 ; |558| 
        ldiu      @CL2,r0               ; |558| 
        or        *+fp(1),ar2           ; |558| 
        callu     r0                    ; far call to _I2C_TX_BYTE	; |558| 
                                        ; |558| Far Call Occurs
        sti       r0,*+fp(7)            ; |558| 
	.line	46
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
        ...
        cmpi      15,r0                 ; |566| 
        bge       L119                  ; |566| 
;*      Branch Occurs to L119           ; |566| 
L118:        
	.line	53
;----------------------------------------------------------------------
; 567 | I2C_RX_BYTE(&p[j*16+i],1);              /* read the bytes , WITH ACK*/ 
;----------------------------------------------------------------------
        ldiu      *+fp(4),ar2           ; |567| 
        ldp       @CL3,DP
        ash       4,ar2                 ; |567| 
        addi      *+fp(2),ar2           ; |567| Unsigned
        ldiu      @CL3,r0               ; |567| 
        addi      *+fp(3),ar2           ; |567| Unsigned
        ldiu      1,r2                  ; |567| 
        callu     r0                    ; far call to _I2C_RX_BYTE	; |567| 
                                        ; |567| Far Call Occurs
	.line	52
        ldiu      1,r0                  ; |566| 
        addi      *+fp(3),r0            ; |566| 
        ...
        cmpi      15,r0                 ; |566| 
        blt       L118                  ; |566| 
;*      Branch Occurs to L118           ; |566| 
L119:        
	.line	55
;----------------------------------------------------------------------
; 569 | I2C_RX_BYTE(&p[j*16+i],0);                      /* the final byte to re
;     | ad, WITH NO ACK */                                                     
;----------------------------------------------------------------------
        ldiu      *+fp(4),ar2           ; |569| 
        ldp       @CL3,DP
        ash       4,ar2                 ; |569| 
        addi      *+fp(2),ar2           ; |569| Unsigned
        ldiu      @CL3,r0               ; |569| 
        addi      *+fp(3),ar2           ; |569| Unsigned
        ldiu      0,r2                  ; |569| 
        callu     r0                    ; far call to _I2C_RX_BYTE	; |569| 
                                        ; |569| Far Call Occurs
	.line	57
;----------------------------------------------------------------------
; 571 | I2C_STOP;                                                              
