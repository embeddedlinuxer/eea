...
;* Date/Time created: Mon Jul 15 11:12:28 2019                                *
;******************************************************************************
...
;******************************************************************************
;	C:\tic3x4x\c3x4x\cgtools\bin\ac30.exe -@_6AO.AAA 
	.file	"dac.c"
...
;----------------------------------------------------------------------
	...
	nop
	nop
        ldfu      0.0000000000e+00,f0   ; |588| 
;*      Branch Occurs to L186           ; |588| 
L160:        
	.line	30
;----------------------------------------------------------------------
; 590 | return 100.0;                                                          
;----------------------------------------------------------------------
	...
	nop
	nop
        ldfu      1.0000000000e+02,f0   ; |590| 
;*      Branch Occurs to L186           ; |590| 
L161:        
	.line	34
;----------------------------------------------------------------------
; 594 | c = *TMR1_CNT;                                                         
;----------------------------------------------------------------------
	...
	.line	37
;----------------------------------------------------------------------
; 597 | if (c >= AO[loop].pid_lastcycle)                                       
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |597| 
        ldp       @CL61,DP
        mpyi      419,ir0               ; |597| 
        ldiu      @CL61,ar0             ; |597| 
        cmpi3     *+ar0(ir0),r0         ; |597| 
        blt       L163                  ; |597| 
;*      Branch Occurs to L163           ; |597| 
	.line	38
;----------------------------------------------------------------------
; 598 | delta = c - AO[loop].pid_lastcycle;                                    
; 599 | else                                                                   
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |598| 
        ldp       @CL61,DP
...
;----------------------------------------------------------------------
	...
	.line	42
;----------------------------------------------------------------------
; 602 | delta = (per - AO[loop].pid_lastcycle) + c;                            
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |602| 
        ldp       @CL61,DP
        mpyi      419,ir0               ; |602| 
...
;----------------------------------------------------------------------
	...
	.line	46
;----------------------------------------------------------------------
; 606 | dt /= TMR1_FREQ;                                /* Clock/2 = 27648000 H
;     | z */                                                                   
;----------------------------------------------------------------------
        ldp       @CL64,DP
        ldiu      @CL64,ar0             ; |606| 
...
;----------------------------------------------------------------------
	...
	.line	49
;----------------------------------------------------------------------
; 609 | if (dt <= 0.0)                                      /* check for underf
;     | low on dt */                                                           
;----------------------------------------------------------------------
        cmpf      0.0000000000e+00,f0   ; |609| 
        bgt       L166                  ; |609| 
;*      Branch Occurs to L166           ; |609| 
	.line	50
;----------------------------------------------------------------------
; 610 | return AO[loop].pid_output;                                            
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |610| 
        ldp       @CL57,DP
        bud       L186                  ; |610| 
        ...
        ldiu      @CL57,ar0             ; |610| 
        ldfu      *+ar0(ir0),f0         ; |610| 
;*      Branch Occurs to L186           ; |610| 
L166:        
	.line	52
;----------------------------------------------------------------------
; 612 | AO[loop].pid_lastcycle  = c;    /* cycle count for next pass */        
; 615 |         //kd                                            = AO[loop].P.va
;     | l;       /* P input */                                                 
; 616 |         //AO[loop].pid_propband = 1.0/(MIN_CYCLE*kd);   /* proportional
;     |  band = 1/(.1*kd)  */                                                  
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |612| 
        ldp       @CL61,DP
	...
	.line	58
;----------------------------------------------------------------------
; 618 | AO[loop].pid_propband   = AO[loop].P.val;       //new PID 9/29/2015    
;----------------------------------------------------------------------
        ldp       @CL68,DP
        ldiu      *+fp(1),ir1           ; |618| 
        ldiu      @CL68,ar1             ; |618| 
	...
	.line	60
;----------------------------------------------------------------------
; 620 | if (AO[loop].pid_propband > MAX_PBAND)                  /* Check if P >
...
;----------------------------------------------------------------------
        ...
        mpyi      419,ir0               ; |620| 
        ldfu      @CL69,f0              ; |620| 
        cmpf3     f0,*+ar0(ir0)         ; |620| 
        ble       L168                  ; |620| 
;*      Branch Occurs to L168           ; |620| 
	.line	61
;----------------------------------------------------------------------
; 621 | AO[loop].pid_propband = MAX_PBAND;                                     
;----------------------------------------------------------------------
        ldp       @CL67,DP
        ldiu      *+fp(1),ir0           ; |621| 
        ldiu      @CL67,ar0             ; |621| 
...
;----------------------------------------------------------------------
	...
	.line	70
;----------------------------------------------------------------------
; 630 | if (AO[loop].I.val <= 0.0)          /* Check if I < 0 ? set to 1000 : d
;     | o nothing*/                                                            
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |630| 
        ldp       @CL70,DP
        mpyi      419,ir0               ; |630| 
        ldiu      @CL70,ar0             ; |630| 
        ldfu      0.0000000000e+00,f0   ; |630| 
        cmpf3     f0,*+ar0(ir0)         ; |630| 
        bgt       L170                  ; |630| 
;*      Branch Occurs to L170           ; |630| 
	.line	71
;----------------------------------------------------------------------
; 631 | AO[loop].I.val  = 10000.0;      /* should set to 10x10^6 */            
;----------------------------------------------------------------------
        ldp       @CL70,DP
        ldiu      *+fp(1),ir0           ; |631| 
        ldiu      @CL70,ar0             ; |631| 
...
;----------------------------------------------------------------------
	...
	.line	75
;----------------------------------------------------------------------
; 635 | if (AO[loop].D.val < 0.0)           /* Check D < 0 ? set to 0 : do noth
;     | ing */                                                                 
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |635| 
        ldp       @CL72,DP
        ...
        bge       L172                  ; |635| 
;*      Branch Occurs to L172           ; |635| 
	.line	76
;----------------------------------------------------------------------
; 636 | AO[loop].D.val = 0.0;                                                  
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |636| 
        ldp       @CL72,DP
        mpyi      419,ir0               ; |636| 
        ldiu      @CL72,ar0             ; |636| 
        ldfu      0.0000000000e+00,f0   ; |636| 
        stf       f0,*+ar0(ir0)         ; |636| 
L172:        
	.line	78
;----------------------------------------------------------------------
; 638 | D  = AO[loop].D.val;                    /*** D ***/                    
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |638| 
        ldp       @CL72,DP
        mpyi      419,ir0               ; |638| 
	...
	.line	80
;----------------------------------------------------------------------
; 640 | last_e  = AO[loop].pid_error;   // get last error                      
; 641 | //last_out = AO[loop].pid_output; // get last output <-- not needed for
...
;----------------------------------------------------------------------
	...
	.line	83
;----------------------------------------------------------------------
; 643 | sp = AO[loop].PID_setpoint;             //setpoint                     
;----------------------------------------------------------------------
        ldp       @CL73,DP
        ldiu      *+fp(1),ir0           ; |643| 
        mpyi      419,ir0               ; |643| 
	...
	.line	85
;----------------------------------------------------------------------
; 645 | e = AO[loop].PID_input - sp;    //current error                        
;----------------------------------------------------------------------
        ldp       @CL59,DP
        ldiu      *+fp(1),ir0           ; |645| 
        mpyi      419,ir0               ; |645| 
        ldiu      @CL59,ar0             ; |645| 
        subf3     f0,*+ar0(ir0),f0      ; |645| 
        stf       f0,*+fp(8)            ; |645| 
	.line	87
;----------------------------------------------------------------------
; 647 | delta_e = e - last_e;                                                  
;----------------------------------------------------------------------
        ldfu      *+fp(9),f0            ; |647| 
        subrf     *+fp(8),f0            ; |647| 
        stf       f0,*+fp(10)           ; |647| 
...
;----------------------------------------------------------------------
	...
	.line	91
;----------------------------------------------------------------------
; 651 | AO[loop].pid_bias += delta_bias;                                       
; 653 | //enforce bias limits                                                  
;----------------------------------------------------------------------
        ldp       @CL13,DP
        ldiu      *+fp(1),ir0           ; |651| 
        mpyi      419,ir0               ; |651| 
        ldiu      415,ar0               ; |651| 
        addi      @CL13,ir0             ; |651| Unsigned
        addf3     f0,*+ar0(ir0),f0      ; |651| 
        stf       f0,*+ar0(ir0)         ; |651| 
	.line	94
;----------------------------------------------------------------------
; 654 | if (AO[loop].pid_bias > 100)                                           
;----------------------------------------------------------------------
        ldp       @CL60,DP
        ldiu      *+fp(1),ir0           ; |654| 
        ldiu      @CL60,ar0             ; |654| 
        mpyi      419,ir0               ; |654| 
        ldfu      1.0000000000e+02,f0   ; |654| 
        cmpf3     f0,*+ar0(ir0)         ; |654| 
        ble       L174                  ; |654| 
;*      Branch Occurs to L174           ; |654| 
	.line	95
;----------------------------------------------------------------------
; 655 | AO[loop].pid_bias = 100;                                               
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |655| 
        ldp       @CL60,DP
        mpyi      419,ir0               ; |655| 
        ldiu      @CL60,ar0             ; |655| 
        ldfu      1.0000000000e+02,f0   ; |655| 
        stf       f0,*+ar0(ir0)         ; |655| 
L174:        
	.line	96
;----------------------------------------------------------------------
; 656 | if (AO[loop].pid_bias < -100)                                          
;----------------------------------------------------------------------
        ldp       @CL60,DP
        ldiu      *+fp(1),ir0           ; |656| 
        ldiu      @CL60,ar0             ; |656| 
        ...
        mpyi      419,ir0               ; |656| 
        ldfu      @CL74,f0              ; |656| 
        cmpf3     f0,*+ar0(ir0)         ; |656| 
        bge       L176                  ; |656| 
;*      Branch Occurs to L176           ; |656| 
	.line	97
;----------------------------------------------------------------------
; 657 | AO[loop].pid_bias = -100;                                              
; 660 | // we multiply output by 10 to keep customer PID values roughly in the
...
L176:        
	.line	101
;----------------------------------------------------------------------
; 661 | new_out = 10*P*(e + AO[loop].pid_bias + (D*delta_e)/dt);               
; 663 | //Don't further accumulate the bias if the output hits 100% or 0%      
;----------------------------------------------------------------------
        ldfu      *+fp(10),f0           ; |661| 
        ldfu      *+fp(11),f1           ; |661| 
        mpyf      *+fp(4),f0            ; |661| 
        ...
        ldfu      *+fp(2),f1            ; |661| 
        addf      *+fp(8),f2            ; |661| 
        mpyf      1.0000000000e+01,f1   ; |661| 
        addf3     f2,f0,f0              ; |661| 
        mpyf3     f0,f1,f0              ; |661| 
        stf       f0,*+fp(6)            ; |661| 
	.line	104
;----------------------------------------------------------------------
; 664 | if ( ((new_out > 100.0) && (delta_bias > 0)) || ((new_out < 0) && (delt
;     | a_bias < 0)) )                                                         
;----------------------------------------------------------------------
        cmpf      1.0000000000e+02,f0   ; |664| 
        ble       L178                  ; |664| 
;*      Branch Occurs to L178           ; |664| 
        ldfu      *+fp(7),f0            ; |664| 
        cmpf      0.0000000000e+00,f0   ; |664| 
        bgt       L180                  ; |664| 
;*      Branch Occurs to L180           ; |664| 
L178:        
        ldfu      *+fp(6),f0            ; |664| 
        cmpf      0.0000000000e+00,f0   ; |664| 
        bge       L181                  ; |664| 
;*      Branch Occurs to L181           ; |664| 
        ldfu      *+fp(7),f0            ; |664| 
        cmpf      0.0000000000e+00,f0   ; |664| 
        bge       L181                  ; |664| 
;*      Branch Occurs to L181           ; |664| 
L180:        
	.line	105
;----------------------------------------------------------------------
; 665 | AO[loop].pid_bias -= delta_bias; //undo the accumulate from earlier    
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |665| 
        ldp       @CL13,DP
        mpyi      419,ir0               ; |665| 
        ldiu      415,ar0               ; |665| 
        addi      @CL13,ir0             ; |665| Unsigned
        ldfu      *+fp(7),f0            ; |665| 
        subf3     f0,*+ar0(ir0),f0      ; |665| 
        stf       f0,*+ar0(ir0)         ; |665| 
L181:        
	.line	108
;----------------------------------------------------------------------
; 668 | if (new_out > 100.0)        // Limit output to between 0 and 100%      
;----------------------------------------------------------------------
        ldfu      *+fp(6),f0            ; |668| 
        cmpf      1.0000000000e+02,f0   ; |668| 
        ble       L183                  ; |668| 
;*      Branch Occurs to L183           ; |668| 
	.line	109
;----------------------------------------------------------------------
; 669 | new_out = 100.0;                                                       
;----------------------------------------------------------------------
        ldfu      1.0000000000e+02,f0   ; |669| 
        stf       f0,*+fp(6)            ; |669| 
        bu        L185                  ; |560| 
;*      Branch Occurs to L185           ; |560| 
L183:        
	.line	110
;----------------------------------------------------------------------
; 670 | else if (new_out < 0.0)                                                
;----------------------------------------------------------------------
        ldfu      *+fp(6),f0            ; |670| 
        cmpf      0.0000000000e+00,f0   ; |670| 
        bge       L185                  ; |670| 
;*      Branch Occurs to L185           ; |670| 
	.line	111
;----------------------------------------------------------------------
; 671 | new_out = 0.0;                                                         
;----------------------------------------------------------------------
        ldfu      0.0000000000e+00,f0   ; |671| 
        stf       f0,*+fp(6)            ; |671| 
L185:        
	.line	113
;----------------------------------------------------------------------
; 673 | AO[loop].pid_output  = new_out;                                        
;----------------------------------------------------------------------
        ldiu      *+fp(1),ir0           ; |673| 
        ldp       @CL57,DP
        mpyi      419,ir0               ; |673| 
