...
#pragma DATA_SECTION(DIAGNOSTICS_MASK,"CFG")
 int DIAGNOSTICS_MASK[2];
 int DIAGNOSTICS_MSG_CODE;
 int DIAGNOSTICS_ERROR_CODE;
 char E_STRING[101];
...
#line 38 "G:\workspace\EEA\V615\APP\SOURCE\INCLUDE\SELFTEST.H"
...
void Set_Diag_Status(void)
{
	...
	if (DIAGNOSTICS[0] & 0x8000)
		DIAGNOSTICS_ERROR_CODE = 16;
	else if ((DIAGNOSTICS[0] & 0x0008) || (DIAGNOSTICS[0] & 0x0010))
		DIAGNOSTICS_ERROR_CODE = 8;
	else if (DIAGNOSTICS[0] & 0x0001)
		DIAGNOSTICS_ERROR_CODE = 10;
	else if (DIAGNOSTICS[0] & 0x0080)
		DIAGNOSTICS_ERROR_CODE = 9;
	else if (REG_WATERCUT.STAT & 0x00000400) 				 
		DIAGNOSTICS_ERROR_CODE = 20;
	 
	else if ((REG_WATERCUT.STAT & 0x00000080) || (DIAGNOSTICS[0] & 0x1000))
		DIAGNOSTICS_ERROR_CODE = 11;    
	else if ((REG_WATERCUT.STAT & 0x00000040) || (DIAGNOSTICS[0] & 0x2000))
		DIAGNOSTICS_ERROR_CODE = 19;
	else if (RTC_PRESENT && (DIAGNOSTICS[1] & 0x0001))
		DIAGNOSTICS_ERROR_CODE = 35;
	 

	if (DIAGNOSTICS_ERROR_CODE == 16)
	{ 
		LED_GREEN 		= 2;
		DIO_ERROR.val 	= 1;
	...
	if ((SYS_LOG_ENABLED & 0x04) && ((DIAGNOSTICS[0] != DIAG_TEMP[0]) || (DIAGNOSTICS[1] != DIAG_TEMP[1])))
	{ 
		DIAG_TEMP[0] = DIAGNOSTICS[0];
		DIAG_TEMP[1] = DIAGNOSTICS[1];
		
		RecordLogEntry(&SYS, 0x04, rec);
	}
	...
	{ 
		...
		case 1:		
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: Frequency");	
			break;	
		...
		case 8:		
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: FLASH Memory");	
			break;	
		...
		case 9:	
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: INT Memory"); 	
			break;	
		...
		case 10:		
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: EXT Memory"); 	
			break;	
			
		case 50:
		{
			if (ANALYZER_MODE.val&0xFF==2)
				sprintf(E_STRING,"ERROR: Invalid PV");	
			else
			{
				if (RUSS)
				{
					...
					sprintf(E_STRING,"ERROR: Invalid WCut");	
			}
		...
		case 11:
		{
			if (ANALYZER_MODE.val&0xFF==2)
				...
				if (RUSS)
				{
					...
					sprintf(E_STRING,"ERROR: Lo Water Cut");	
			break;
		}
		case 20:
		case 6:
		case 19:
		{
			...
			else
			{
				if (RUSS)
				{
					...
					sprintf(E_STRING,"ERROR: Hi Water Cut");	
			}
		...
		case 16:	
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: RESET BASICS"); 	
			break;	
		case 25:	
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: Low Temp."); 	
			break;	
		case 26:	
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: High Temp."); 	
			break;	
		case 35:	
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: Invalid Time");	
			break;	
		case 28:		
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR: Phase Logic"); 	
			break;	
		default:		
			if (RUSS)
			{
				...
				sprintf(E_STRING,"ERROR #%02d",DIAGNOSTICS_ERROR_CODE); 
			break;
	}
...
void Self_Test_FULL(void)
{
	...
	if (Calc_CRC(((unsigned int*)0x418000), 0x00008000))	 
		DIAGNOSTICS[0] |= 0x0008;

	if (Calc_CRC(((unsigned int*)0x408000), 0x00008000))			 
		DIAGNOSTICS[0] |= 0x0008;

	CRC = Calc_CRC(((unsigned int*)0x400000), 0x00002000);		 

	if (CRC!=CRC_BOOT)
		DIAGNOSTICS[0] |= 0x0008;

	CRC = Calc_CRC(((unsigned int*)0x428000), 0x00008000);		 

	if (CRC!=CRC_FAST)
		DIAGNOSTICS[0] |= 0x0008;

	CRC = Calc_CRC(((unsigned int*)0x430000), 0x00030000);			 

	if (CRC!=CRC_APP)
		DIAGNOSTICS[0] |= 0x0008;

	CRC = Calc_CRC(((unsigned int*)0x460000), 0x00010000);		 
...
void Self_Test5(void)
{
	...
	{ 
		...
		TMR_SELFTEST.vect = (int*)Self_Test1;			 
	}

	Timer_Insert(&TMR_SELFTEST,0,2);
...
void Self_Test4(void)
{
	...
	{ 
		...
		TMR_SELFTEST.vect = (int*)Self_Test5;				 
	}
...
void Self_Test3(void)
{
	...
	{ 
		...
		TMR_SELFTEST.vect = (int*)Self_Test4;				 
	}
...
void Self_Test2(void)
{
	...
	{ 
		...
		TMR_SELFTEST.vect = (int*)Self_Test3;			 
	}
...
void Self_Test1(void)
{
	...
	{ 
		...
		TMR_SELFTEST.vect = (int*)Self_Test2;					 
	}
